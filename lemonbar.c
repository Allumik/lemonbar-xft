...
// rotation part from venam's fork
xcb_render_fixed_t float_to_fixed(float f) { return f * 0x10000; }

xcb_rectangle_t get_drawable_size(xcb_connection_t *c,
...
uint8_t get_drawable_depth(xcb_connection_t *c, xcb_drawable_t drawable) {
  xcb_get_geometry_cookie_t cookie;
  xcb_generic_error_t *e;
  xcb_get_geometry_reply_t *geom;
  uint8_t depth = 0;
...
void rotate_pixmap(xcb_connection_t *c, xcb_pixmap_t from, xcb_pixmap_t to,
  ...
  xcb_render_composite_checked(
      c, XCB_RENDER_PICT_OP_SRC, picture, pic_mask, back_pix,
      (rotate_text == 2 ? 0 : -p_size.width), // shifted by
      (rotate_text == 1 ? 0 : -p_size.height), 0, 0, 0, 0, p_size.width,
      p_size.height);
}
...
void set_ewmh_atoms(void) {
  ...
  for (monitor_t *mon = monhead; mon; mon = mon->next) {
    ...
    xcb_ewmh_set_wm_strut(ewmh, mon->window, 0, 0, strut.top, strut.bottom);
...
monitor_t *monitor_new(int x, int y, int width, int height) {
  ...
  if (rotate_text == 0) {
    ...
    xcb_create_window(
        c, depth, ret->window, scr->root, ret->x, ret->y, bh, ret->width, 0,
        XCB_WINDOW_CLASS_INPUT_OUTPUT, visual,
        XCB_CW_BACK_PIXEL | XCB_CW_BORDER_PIXEL | XCB_CW_OVERRIDE_REDIRECT |
            XCB_CW_EVENT_MASK | XCB_CW_COLORMAP,
        (const uint32_t[]){
...
void init(char *wm_name, char *wm_instance) {
  ...
  if (qe_reply && qe_reply->present) {
    ...
    if (qe_reply && qe_reply->present) {
      ...
      xia_reply =
          xcb_xinerama_is_active_reply(c, xcb_xinerama_is_active(c), NULL);
  ...
  for (monitor_t *mon = monhead; mon; mon = mon->next) {
    ...
    if (wm_instance) {
      ...
      xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window,
                          XCB_ATOM_WM_CLASS, XCB_ATOM_STRING, 8, wm_class_len,
                          wm_class);
...
int main(int argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv, "hg:brRdf:a:pu:B:F:U:n:o:")) != -1) {
    switch (ch) {
    case 'h':
      printf(
          "lemonbar version %s patched with XFT support and vertical transform "
          "support \n",
          VERSION);
      printf("usage: %s [-h | -r | -R | -g | -b | -d | -f | -a | -p | -n | -u "
             "| -B | -F]\n"
             "\t-h Show this help\n"
  ...
  if (areas) {
    ...
    if (!area_stack.area) {
      fprintf(stderr,
              "Could not allocate enough memory for %d clickable areas, try "
              "lowering the number\n",
              areas);
      return EXIT_FAILURE;
    }
        ...
        while ((ev = xcb_poll_for_event(c))) {
          ...
          case XCB_BUTTON_PRESS:
            press_ev = (xcb_button_press_event_t *)ev;
            {
              area_t *area = NULL;
              if (rotate_text == 0) {
                area = area_get(press_ev->event, press_ev->detail,
                                press_ev->event_x);
                ...
                area = area_get(press_ev->event, press_ev->detail,
                                monhead->width - press_ev->event_y);
              }
    ...
    if (redraw) { // Copy our temporary pixmap onto the window
      for (monitor_t *mon = monhead; mon; mon = mon->next) {
        if (rotate_text != 0) {
          ...
          xcb_create_pixmap(c, mon->depth, rotate_pmap, mon->window, bh,
                            mon->width);
          xcb_change_gc(c, gc[GC_DRAW], XCB_GC_FOREGROUND,
                        (const uint32_t[]){fgc.v});
          xcb_poly_fill_rectangle(
              c, rotate_pmap, gc[GC_DRAW], 1,
              (const xcb_rectangle_t[]){{0, 0, bh, mon->width}});
          rotate_pixmap(c, mon->pixmap, rotate_pmap, fmtid);
          xcb_copy_area(c, rotate_pmap, mon->window, gc[GC_DRAW], 0, 0, 0, 0,
                        bh, mon->width);
          xcb_free_pixmap(c, rotate_pmap);
        } else {
          xcb_copy_area(c, mon->pixmap, mon->window, gc[GC_DRAW], 0, 0, 0, 0,
                        mon->width, bh);
        }
      }
    }
